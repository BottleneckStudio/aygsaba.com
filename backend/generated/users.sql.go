...
func (q *Queries) CreateUser(ctx context.Context, arg CreateUserParams) (User, error) {
	row := q.db.QueryRowContext(ctx, createUser, arg.ID, arg.Email, arg.CreatedAt)
	var i User
	err := row.Scan(
		&i.ID,
		&i.Email,
...
SELECT id, email, avatar, created_at FROM users
WHERE id = ? LIMIT 1
`
...
func (q *Queries) UpdateAvatar(ctx context.Context, arg UpdateAvatarParams) (User, error) {
	row := q.db.QueryRowContext(ctx, updateAvatar, arg.Avatar, arg.ID, arg.Email)
	var i User
	err := row.Scan(
		&i.ID,
		&i.Email,
